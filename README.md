...
- **🌐 大模型翻译功能** - 服务端代理调用 Gemini API，避免前端暴露密钥
- **⚡ 高性能优化** - 30分钟智能缓存、文章预加载、翻译 SDK 按需加载
- **📱 完美响应式设计** - 移动端单页面切换，桌面端侧边栏布局
- **⚙️ 灵活配置管理** - 支持自定义仓库地址、分支、路径和文件夹过滤
- **🎨 现代化 UI** - Toast 通知、骨架屏加载、平滑动画、错误提示处理
...
```
api/               # Vercel Serverless Functions（服务端 API 代理）
src/
├── components/    # React 组件（features/layout/ui 分类）
├── services/      # 前端业务逻辑层（GitHub API、配置、翻译调用）
...
2. 配置 `GEMINI_API_KEY`（服务端使用，不要加 `VITE_` 前缀）
3. 可选配置 `GEMINI_MODEL`

> 注意：`VITE_*` 变量会打包到客户端，不可用于生产密钥。请只在服务端环境变量中配置 `GEMINI_API_KEY`。
